...
@test "1. Create secret using command arguments" {
    run bash -c "./conductor secret put '$SECRET_KEY' '$SECRET_VALUE' 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "6. Create secret using --value flag" {
    ...
    # Verify value
    run bash -c "./conductor secret get '$SECRET_KEY_2' --show-value 2>&1"
    echo "$output" | grep -q "$SECRET_VALUE"
}

@test "7. Create secret from stdin" {
    run bash -c "echo '$SECRET_VALUE' | ./conductor secret put '$SECRET_KEY' 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
    echo "$output" | grep -q "saved successfully"
...
@test "14. Clear local cache" {
    run bash -c "./conductor secret cache-clear --local 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "15. Clear Redis cache" {
    run bash -c "./conductor secret cache-clear --redis 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "16. Clear both caches (no flags)" {
    run bash -c "./conductor secret cache-clear 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
    echo "$output" | grep -q "Local cache cleared successfully"
    echo "$output" | grep -q "Redis cache cleared successfully"
...
@test "19. Error handling - get non-existent secret" {
    ...
    run bash -c "./conductor secret get 'non_existent_secret_xyz' 2>&1"
    echo "Output: $output"
    [ "$status" -ne 0 ]
}
...
@test "20. Error handling - delete non-existent secret" {
    ...
    run bash -c "./conductor secret delete 'non_existent_secret_xyz' -y 2>&1"
    echo "Output: $output"
    [ "$status" -ne 0 ]
}
