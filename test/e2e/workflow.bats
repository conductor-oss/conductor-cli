...
@test "2. Get workflow definition" {
    run bash -c "./conductor workflow get '$WORKFLOW_NAME' 2>/dev/null"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "3. Create workflow definition for sync test" {
    run bash -c "./conductor workflow create '$WORKFLOW_FILE_2' --force 2>/dev/null"
    echo "Output: $output"
    [ "$status" -eq 0 ]
}
...
@test "8. Terminate workflow execution" {
    ...
    run bash -c "./conductor workflow terminate '$WORKFLOW_ID' 2>/dev/null"
    echo "Output: $output"
    [ "$status" -eq 0 ]
