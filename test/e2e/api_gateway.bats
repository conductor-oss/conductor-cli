...
setup() {
    ...
    ./conductor workflow create test/e2e/test-workflow-2.json --force 2>/dev/null || true
}
...
teardown() {
    ...
    ./conductor api-gateway service delete "$SERVICE_ID" -y 2>/dev/null || true
    ./conductor api-gateway auth delete "$AUTH_CONFIG_ID" -y 2>/dev/null || true
}

# ==================== Auth Config Tests ====================

@test "1. Create auth config using flags" {
    run bash -c "./conductor api-gateway auth create \
...
@test "2. Get auth config by ID" {
    ...
    run bash -c "./conductor api-gateway auth get '$AUTH_CONFIG_ID' 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "3. List auth configs includes created config" {
    ...
    run bash -c "./conductor api-gateway auth list 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "4. Create service using flags" {
    ...
    run bash -c "./conductor api-gateway service create \
...
@test "5. Get service by ID" {
    ...
    run bash -c "./conductor api-gateway service get '$SERVICE_ID' 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "6. List services includes created service" {
    ...
    run bash -c "./conductor api-gateway service list 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
EOF
    ...
    run bash -c "./conductor api-gateway service update '$SERVICE_ID' /tmp/service_update.json 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
EOF
    ./conductor api-gateway service update "$SERVICE_ID" /tmp/service_update.json 2>/dev/null
...
@test "9. Create route using flags" {
    ...
    run bash -c "./conductor api-gateway route create '$SERVICE_ID' \
        --http-method 'GET' \
        --path '/test' \
...
@test "10. List routes for service" {
    ...
    ./conductor api-gateway service create \
        ...
        --path "/api/e2e" \
        --enabled 2>/dev/null || true
    ./conductor api-gateway route create "$SERVICE_ID" \
        --http-method "GET" \
        --path "/test" \
        --workflow-name "$WORKFLOW_NAME" \
        --workflow-version 1 \
    ...
    run bash -c "./conductor api-gateway route list '$SERVICE_ID' 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
EOF
    ...
    run bash -c "./conductor api-gateway route update '$SERVICE_ID' '/update-test' /tmp/route_update.json 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
    echo "$output" | grep -q "updated successfully"
}
...
@test "13. Delete route" {
    ...
    run bash -c "./conductor api-gateway route delete '$SERVICE_ID' 'DELETE' '/delete-test' -y 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
...
@test "14. Delete service" {
    ...
    run bash -c "./conductor api-gateway service delete '$SERVICE_ID' -y 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
    echo "$output" | grep -q "deleted successfully"
}
...
@test "16. Delete auth config" {
    ...
    run bash -c "./conductor api-gateway auth delete '$AUTH_CONFIG_ID' -y 2>&1"
    echo "Output: $output"
    [ "$status" -eq 0 ]
    echo "$output" | grep -q "deleted successfully"
}
